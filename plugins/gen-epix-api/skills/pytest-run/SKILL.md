...
1. **Capture once** to a fixed, gitignored path under `tmp/` (already gitignored in
   ...
   ```bash
   pytest test -v --tb=long -ra > tmp/pytest-run.log 2>&1; echo "exit: $?"
   ```

   - Swap `test` for a narrower target (a file, `-k EXPR`, a marker) matching what
     the user actually asked to test — this is still a single capture.
...
- Still finish with the repo's real verification step before calling work done —
  ...
  `pytest test -x -q` run per the session conventions.
